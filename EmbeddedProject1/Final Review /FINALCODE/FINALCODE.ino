...
#define CMPS14_ADDRESS  0x60 // I2C address for CMPS14 compass
...
// Motor/Encoder/Compass Variables
volatile long encoderCount_left = 0;
volatile long encoderCount_right = 0;
float distperpuls = 1.3; // 1.3 pulses per mm (or 0.13 pulses per cm)
volatile float bearingDegrees = 0;
int bearingMaxError = 2; 
int tuningSpeed = 20;    
bool motorRunning = false;
int joyX_val = 0; // To store raw joystick readings for LCD
int joyY_val = 0;

// Travel Plan
int travelPlan[1][5] = {
  {75,0,'f',0,'l'} // {Speed%, Distance(mm), 'f'/'b', Angle(deg), 'l'/'r'}
};
...
void updateScreen(){
  ...
  // --- ROW 1 ---
  // [ ] Compass: For both modes, display compass degree and direction (N, NE, E…)
