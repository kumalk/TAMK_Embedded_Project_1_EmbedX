...
int travelPlan[][4] = {
  {50,200,'f',50},{50,200,'f',170},{75,200,'f',290}
  //{50,200,'f',50},{25,50,'f',170},{75,100,'f',290}
};
...
void loop() {
  if (buttonPressedFlag) {
    ...
    runMotors(travelPlan[currentTravelSection][0],travelPlan[currentTravelSection][2],travelPlan[currentTravelSection][3]);
    motorRunning = true;
  }
...
void runMotors(int speedPercent, char direction, int targetBearing) {
  turnToBearing(targetBearing); // make sure car faces correct direction first
  encoderCount_left = 0;
  encoderCount_right = 0;
...
void turnToBearing(int targetBearing) {
  while (!isBearingCorrect(targetBearing)) {
    char dir = turningDirectionDecider(targetBearing);
...
char turningDirectionDecider(int target) {
  ...
  // --- Dead zone (tolerance) ---
  int bearingMaxError = 4;

  // --- Decide turning direction ---
  if (abs(diff) <= bearingMaxError) {
    return 's';  // stop
  } else if (diff > 0) {
    return 'r';  // turn right
  } else {
...
//   Serial.println(abs(diff) <= 10);
//   return abs(diff) <= 10;  // tolerance
