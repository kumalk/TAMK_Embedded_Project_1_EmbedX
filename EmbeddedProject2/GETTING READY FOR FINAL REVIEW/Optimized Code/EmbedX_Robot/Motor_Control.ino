...
// --- Motor Control Functions (ESP Mode) ---
void goMotors(int speedPercent, int goDistance, char goDirection, int rotationAngle, char rotationDirection) {
  
  Serial.println("Go Flag Found");
  speedPercent=25;
  // 3. Perform the turn first
  turnToBearing(targetBearingCal(rotationAngle, rotationDirection), rotationDirection); 

   Serial.println("Rotated");

  // 4. Set motor directions
  if (goDirection == 'f') {
    digitalWrite(Motor_L_dir_pin, Motor_forward);
    digitalWrite(Motor_R_dir_pin, Motor_forward);
    tartgetDistance = currentDistance - goPlan[1];
  } else {
    digitalWrite(Motor_L_dir_pin, Motor_return);
    digitalWrite(Motor_R_dir_pin, Motor_return);
    tartgetDistance = currentDistance + goPlan[1];
