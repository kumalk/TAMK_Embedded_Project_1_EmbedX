...
if(pathFlag){
    ...
    Serial.print("  |  ");
    Serial.print(bearingDegrees);
    Serial.print("  |  ");
    Serial.println(bearingDegrees-LockedbearingDegrees);
   if(abs(bearingDegrees-LockedbearingDegrees)<0.5){
    if(currentDistance > GapValue+ MaxErrGap){
      //run motor forward
      if(currentDistance >GapValue+  3*MaxErrGap){
      //run motor forward slowly
    ...
    char correctBearingDirection; 
    getRotationAngle(LockedbearingDegrees,correctBearingDirection);
    turnToBearing(LockedbearingDegrees,correctBearingDirection);
    handleFollowPath();
   }
