...
let currentSpeedVal = document.getElementById('speedVal') ? document.getElementById('speedVal').value : 75;
const errMsg = document.getElementById('speedWarninng');
let currentDirVal = document.getElementById('dirVal') ? document.getElementById('dirVal').value : 0;
const dirErrMsg = document.getElementById('dirWarninng');
...
setInterval(fetchColorData, 100); // Fetch Color data every half second
...
async function fetchCompassData() {
  ...
  async function fetchColorData() {
    try {
      ...
      const data = await response.text();
      // Use parseFloat if you want decimals, or keep parseInt for whole numbers
      const colorHashValue = data;
  
      // FIX: Change lidarText.innerText to compassText.innerText
      if (colorBox) colorBox.style.backgroundColor = `${colorHashValue}`; 
...
function speedUpdate(speedVal){
	if (speedVal >= 0 && speedVal <= 100){
		currentSpeedVal = speedVal;
		console.log(currentSpeedVal);
		if(errMsg) errMsg.style.display='none';
		fetch(`/newspeed?value=${speedVal}`);	
	}else{
		if(errMsg) errMsg.style.display='block';
	}
}

function dirUpdate(dirVal){
	if (dirVal >= 0 && dirVal <= 359){
		currentDirVal = dirVal;
		console.log(dirVal);
		if(dirErrMsg) dirErrMsg.style.display='none';
		fetch(`/todir?value=${dirVal}`);	
	}else{
		if(dirErrMsg) dirErrMsg.style.display='block';
	}
}